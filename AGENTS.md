...
- **Hosting Platform**: CasaOS (running in Docker container)
- **Infrastructure**: LXC Container in Proxmox
- **Access URL**: http://lenovo.casaos.local:4096/
- **Architecture**: Nested containerization (Proxmox LXC → CasaOS Docker → OpenCode services)

**Important implications:**
- Network access goes through both LXC and Docker networking layers
- DNS resolution uses `lenovo.casaos.local` for local network discovery
- Port 4096 is exposed through CasaOS to access OpenCode interface
- Services must account for potential network latency from nested containerization
- Container restart operations must be handled through CasaOS management interface
